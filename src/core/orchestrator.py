"""
Security orchestrator - coordinates all security checks
"""
...
class PromptValidator:
    ...
    def __init__(self):
        ...
        if self.config.prompt_injection.enabled and self.config.prompt_injection.layers:
            llm_enabled = any(
                (isinstance(layer, dict) and layer.get("name") == "llm" and layer.get("enabled"))
                or (isinstance(layer, str) and layer == "llm")
                for layer in self.config.prompt_injection.layers
            )
            if llm_enabled:
                try:
