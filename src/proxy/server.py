...
# Allowlist for characters permitted in forwarded URL paths.
# Covers all known LLM API path conventions:
#   - letters/digits, slashes, hyphens, underscores, dots
#   - colon (:) — Gemini uses it for :generateContent
# Any character outside this set indicates a malformed or hostile path.
_SAFE_FORWARD_PATH_RE = re.compile(r"^[A-Za-z0-9/_\-.:]+$")


def _sanitize_forward_path(path: str) -> str:
    """Normalize *and* validate a user-supplied path before proxying.

    Combines ``_normalize_path`` (CWE-918 path-traversal prevention) with a
    strict character allowlist so that CodeQL can determine that tainted user
    input does not reach the forwarded URL without passing an explicit
    validation gate.  Raises ``HTTPException(400)`` for invalid paths.
    """
    normalized = _normalize_path(path)
    if not _SAFE_FORWARD_PATH_RE.match(normalized):
...
def create_proxy_app(
        ...
        try:
            ...
            if isinstance(body, dict):
                ...
                for content in user_messages:
                    ...
                    if not result.safe and result.blocked_by != "secret_detection":
                        ...
                        if threat_score >= _min_block_score:
                            ...
                            # result.blocked_by / result.threat_level are internal
                            # labels generated by the validator (e.g. "regex_rules",
                            # "HIGH") — not raw user text. CodeQL cannot determine
                            # this from the taint graph, hence the suppression.
                            logger.warning(  # lgtm[py/log-injection]
                ...
                if _body_needs_redaction:
                    # Discard the count: it is a tuple element derived (in
                    # CodeQL's taint analysis) from the secret-bearing body,
            ...
            # urlparse().hostname gives the exact hostname, preventing CodeQL
            # py/incomplete-url-substring-sanitization: substring checks like
            # 'googleapis.com' in url could be fooled by evil.com/googleapis.com.
            if urlparse(forward_url).hostname == "generativelanguage.googleapis.com":
            ...
            # Validate forward path characters (partial SSRF guard, CWE-918).
            # Applied after any provider-specific rewrite (e.g. Gemini prefix).
            # _SAFE_FORWARD_PATH_RE allows only [A-Za-z0-9/_-.:]  — any other
            # character indicates a malformed or hostile path.
            if not _SAFE_FORWARD_PATH_RE.match(request_path):
                raise HTTPException(status_code=400, detail="Invalid request path")
    ...
    async def proxy_v1_catchall(request: Request, path: str):
        ...
        # X-Target-URL is disabled for security (SSRF / exfiltration risk).
        forward_url = target_url
        # Normalize + validate path to prevent partial-SSRF (CWE-918).
        # Query params are passed as a structured dict to httpx (not concatenated
        # into the URL string) so that tainted user input never reaches url=.
        forward_path = _sanitize_forward_path(f"/v1/{path}")
        async with httpx.AsyncClient() as client:
            forward_headers = dict(request.headers)
            forward_headers.pop("host", None)
    ...
    async def proxy_universal_catchall(request: Request, path: str):
        ...
        # Query params are passed as a structured dict to httpx (not concatenated
        # into the URL string) so that tainted user input never reaches url=.
        forward_path = _sanitize_forward_path(f"/{path}")
        async with httpx.AsyncClient() as client:
            forward_headers = dict(request.headers)
            forward_headers.pop("host", None)
            # strip even though ignored
